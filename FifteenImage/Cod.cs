...
namespace FifteenImage
{
    //Класс управляющий работой алгоритмом IDA*  
        internal sealed class IDA : SearchBase
        {
            ...
            {
                _start = start;//начальное состояние
            }
            #endregion
            #region Variables
            ...
            class State
            {
                ...
                //линейная функция проверки является ли данное состояние целевым
                /*
                public bool IsFinal()
                {
                    int i, j, start;
           ...
           class SearchIDA
            {
                    ...
                    int emptyXOffset, int emptyYOffset, bool edge, int depthDistributeStep)
                {
                    ...
                    else if (emptyY != 0 && //Если перемещение пустышки влево возможно 
                             1 != emptyYOffset && //и если мы не делаем возвратное перемещение
                             DoMakeStep(emptyX, emptyY, 0, -1, edge, depthDistributeStep))//то сделаем его
                    {
                        _solution.Add(new Move(emptyX, emptyY - 1));
                    ...
                    else if (emptyX != SearchBase.Dimension - 1 && //Если перемещение пустышки вниз возможно 
                    ...
                    else if (emptyX != 0 && //Если перемещение пустышки вверх возможно 
                            1 != emptyXOffset && //и если мы не делаем возвратное перемещение
                             DoMakeStep(emptyX, emptyY, -1, 0, edge, depthDistributeStep))//то сделаем его
                    {
                        _solution.Add(new Move(emptyX - 1, emptyY));
                    ...
                    else //Все 4 перемещения или невозможны или ухудшают состояние 
                        return false;
                }
                /// <summary>
                    ...
                    , int emptyX, int emptyY, int emptyXOffset, int emptyYOffset)
                {
                    ...
                    {
                        int posMustBe = (int)(cell / SearchBase.Dimension);//целевая позиция ячейки
                        ...
                        if (posMustBe == emptyX)
                            ...
                            else//иначе если целевая позиция ячейки находится внизу
                                //то вернём истину если мы производим перемещение 
                                //пыстышки вверх (=> ячейки вниз), т.е. мы стремимся к цели
                                return emptyXOffset > 0;
                    }
                    else //мы производим перемещения по оси Y
                    {
                        ...
                        if (posMustBe == emptyY)
                            ...
                            else//иначе если целевая позиция находится слева
                                //то вернём истину если мы производим перемещение пыстышки вправо 
                                //(=> ячейки влево) , т.е. мы стремимся к цели
                                return emptyYOffset > 0;
                    }
                }
                /// <summary>
                    ...
                    int emptyYOffset, bool edge, int depthDistributeStep)
                {
                    ...
                    if (isGoodMove)
                    {
                        ...
                        if (edge &&//мы находимся в состоянии поиска с улучшающимися положениями 
                            SearchBase.Dimension - 1 == newEmptyX &&//если пустышка на месте по оси X
                            SearchBase.Dimension - 1 == newEmptyY &&//если пустышка на месте по оси Y
                    ...
                    else //Мы не находимся в состояния выхода из локального экстремума,
                        //а данное перемещение ухудшает состояние 
                        return false;
                }
                #endregion
            };
            ...
            protected override void ShowStatistics()
            {
                while (thread != null && thread.IsAlive)//(arResult == null)
                {
                    ...
                    Progressing(
                        "Текущее кол-во исследованных состояний\t: " + _countStates.ToString(CultureInfo.InvariantCulture) +
                        Environment.NewLine + "Время поиска\t\t\t\t: " + timeSearch.ToString() +
                        Environment.NewLine + "Текущая максимальная глубина поиска \t: " + _depthDistributeStep.ToString(CultureInfo.InvariantCulture));
                    Thread.Sleep(200);
                }
            }
            ...
            protected override void StartSearch()
            {
               ...
               /* if (!DoHaveResolve(_start))
                    Progressing("Решения не имеет. Матрица не чётная");
                else*/
                {
